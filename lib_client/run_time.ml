...
let timestamps_from_job_info (ji : Client.job_info) : ((timestamps, string) result) =
  ...
  | Active ->
    ...
    ~some:(fun q ->
      ...
      ~some:(fun s : (timestamps, string) result -> Ok (Running {ready=q; started=s})))
  | Passed | Failed _ | Aborted ->
    ...
    ~some:(fun q ->
      ...
      ~some:(fun f : (timestamps, string) result -> Ok (Finished {ready=q; started=ji.started_at; finished=f})))
  | Undefined _ ->
    Error "Outcome is Undefined - cannot construct timestamp."

let from_jobs (lji : Client.job_info list) =
  lji
  |> List.map (fun ji -> Result.to_option @@ timestamps_from_job_info ji)
