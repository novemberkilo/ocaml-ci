...
module Make (M : Git_forge_intf.Forge) = struct
  ...
  let list ~org ~repo ~message ~refs ~hash ~jobs ~first_step_queued_at
    ...
    in
    let title_card =
      let ref_path = try Some (Result.get_ok (M.ref_path (List.hd refs))) with _ -> None in
      let history_url = Option.bind ref_path (fun ref_path -> Some (Url.history_url M.prefix ~org ~repo ~ref:ref_path)) in
